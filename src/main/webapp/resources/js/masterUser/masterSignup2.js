...
signButton.addEventListener("click", function (event) {
  if (!validation()) {
    ...
    console.log("회원가입 처리");
    arrayValue[2] = document.getElementById('username').value;
    arrayValue[3] = genderValue;
    arrayValue[4] = document.getElementById('useremail').value;
    arrayValue[5] = document.getElementById('userpw').value;
    arrayValue[6] = document.getElementById('userphone').value;
	console.log("배열 : "+arrayValue);
  try{
    const url = '/masterUser/masterSignup';
    const config = {
        method: "post",
...
function man(){
  document.getElementById('man').style.color = "white";
  document.getElementById('woman').style.color = "black";
  document.getElementById('man').style.backgroundColor = "#00c7ae";
...
function woman(){
  ...
  document.getElementById('woman').style.backgroundColor = "#00c7ae";
  genderValue = "여자";
  checkInputs();
}
